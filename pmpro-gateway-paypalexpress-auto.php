...
define( 'PMPRO_PPEA_VERSION', '1.4.3' );
...
class PMProGateway_paypalexpress_auto extends PMProGateway {
	...
	static function init() {
		...
		if ( "paypalexpress_auto" === $gateway ) {
			...
			/**
			 * @since 1.4.4 - Enable using TLS 1.2 on connect w/PayPal gateway
			 */
			add_action( 'http_api_curl', array( $gw, 'enable_tls_12' ), 10, 3 );
			...
			/*
			Filter pmpro_next_payment to get actual value via the PayPal API. This is disabled by default
			for performance reasons, but you can enable it by copying this line into a custom plugin:

			add_filter('pmpro_paypal_next_payment_enabled', '__return_true', 10);
			*/
			/**
			 * Do we enable next-payment lookup at PayPal.com gateway (rather than local calculation)?
			 * @since v1.4.4 - Added filter 'pmpro_paypal_next_payment_enabled' (default: false)
			 */
			if ( true === apply_filters( 'pmpro_paypal_next_payment_enabled', false ) ) {

				add_filter( 'pmpro_next_payment', array( $gw, 'pmpro_next_payment' ), 10, 3 );
			}
	 ...
	 * @since 1.4.4
	 */
	static function enable_tls_12( $handle, $r, $url ) {
